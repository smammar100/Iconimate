...
/* ── Homepage code-splitting modules ────────────────────────────────────────
   The homepage (grid, hero, ⌘K palette) must not import the icons barrel —
   its 159 static imports all land in the initial bundle and cost ~1.6s of
   script evaluation on mobile. Instead we emit:
   - registry/icon-meta.gen.ts   slug/name/keywords only (SSR text, search)
   - registry/lazy-icons.gen.tsx React.lazy per icon, so each icon is its own
                                 chunk that hydrates progressively.
   Both are derived from registry/icons/index.ts, which stays the single
   hand-maintained source of truth (and keeps serving docs/lab routes). */

const indexSrc = read(join(ICONS_DIR, "index.ts"));

// slug -> exported component name, from the index's own import lines.
const exportNames = new Map();
for (const m of indexSrc.matchAll(/^import \{ (\w+) \} from "\.\/([\w-]+)";$/gm)) {
  exportNames.set(m[2], m[1]);
}
const missingExports = slugs.filter((s) => !exportNames.has(s));
if (missingExports.length) {
  throw new Error(`icons missing import lines in index.ts: ${missingExports.join(", ")}`);
}

// Hidden-on-home slugs, from the hand-curated HOME_HIDDEN_SLUGS set.
const hiddenBlock = indexSrc.match(/HOME_HIDDEN_SLUGS = new Set<string>\(\[([\s\S]*?)\]\)/);
if (!hiddenBlock) throw new Error("HOME_HIDDEN_SLUGS not found in index.ts");
const hiddenSlugs = [...hiddenBlock[1].matchAll(/"([^"]+)"/g)].map((m) => m[1]);

const GEN_HEADER = "// AUTO-GENERATED by scripts/build-registry.mjs — do not edit.\n";

/**
 * Recency, for the palette's "Recently Added" section.
 *
 * NOT the order of index.ts. New icons are appended there, so the tail LOOKS
 * chronological, but a batch lands alphabetically and the position only records
 * when a FILE was created — not when its animation was authored. Deriving from
 * the tail put binoculars/binary/bicycle on the shelf while heart, star and trash,
 * the three most recently worked on, never appeared at all.
 *
 * Git's last-commit date per file is the honest signal, with one correction: a
 * file with uncommitted edits is being worked on right now, so it outranks its own
 * stale commit date. mtime alone can't be the primary source — a fresh clone gives
 * every file the same checkout time, which would scramble this in CI.
 *
 * If git isn't available at all (tarball install, some sandboxes), every icon gets
 * timestamp 0 and the consumer falls back to index order — the old behaviour,
 * rather than a crash.
 */
function iconRecency(slugList) {
  let dirty = new Set();
  try {
    const out = execFileSync("git", ["status", "--porcelain", "--", "registry/icons"], {
      cwd: ROOT,
      encoding: "utf8",
    });
    dirty = new Set(
      out
        .split("\n")
        .map((l) => l.slice(3).trim().match(/registry\/icons\/([\w-]+)\.tsx$/)?.[1])
        .filter(Boolean),
    );
  } catch {
    /* not a git checkout — fall through, everything scores 0 */
  }
  ...
  for (const slug of slugList) {
    if (dirty.has(slug)) {
      stamps.set(slug, now); // in-flight work sorts newest
      continue;
    }
    try {
      const t = execFileSync("git", ["log", "-1", "--format=%ct", "--", `registry/icons/${slug}.tsx`], {
        cwd: ROOT,
        encoding: "utf8",
      }).trim();
      stamps.set(slug, t ? Number(t) : 0);
    } catch {
      stamps.set(slug, 0);
    }
  }
...
${[...entries.entries()]
  .map(([slug, e]) => `  { slug: ${JSON.stringify(slug)}, name: ${JSON.stringify(e.name)}, keywords: ${JSON.stringify(e.keywords)} },`)
  .join("\n")}
];

export const HOME_HIDDEN_SLUGS = new Set<string>(${JSON.stringify(hiddenSlugs)});
...
${slugs
  .map((slug) => `  ${JSON.stringify(slug)}: lazy(() => import(${JSON.stringify(`./icons/${slug}`)}).then((m) => ({ default: m.${exportNames.get(slug)} }))),`)
  .join("\n")}
