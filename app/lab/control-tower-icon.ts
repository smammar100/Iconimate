/**
 * Phosphor air-traffic control-tower glyph (filled — render with `fill="currentColor"`).
 * Split into the static base + the antenna so the antenna can animate on its own.
 */

/** The whole tower (antenna + control room + legs). */
export const TOWER =
  "M229.11,70.82A16,16,0,0,0,216,64H136V32h16a8,8,0,0,0,0-16H104a8,8,0,0,0,0,16h16V64H40A16,16,0,0,0,25,85.47l26.19,72a16,16,0,0,0,15,10.53H96v64a8,8,0,0,0,16,0V168h32v64a8,8,0,0,0,16,0V168h29.82a16,16,0,0,0,15-10.53l26.19-72A16,16,0,0,0,229.11,70.82ZM110.68,152,97.58,80h60.84l-13.1,72ZM40,80H81.32l13.09,72H66.18Zm149.82,72H161.59l13.09-72H216Z";

/** Control room + legs with a straight top bar (antenna removed) — the static base for the antenna flip. */
export const TOWER_BASE =
  "M229.11,70.82A16,16,0,0,0,216,64H40A16,16,0,0,0,25,85.47l26.19,72a16,16,0,0,0,15,10.53H96v64a8,8,0,0,0,16,0V168h32v64a8,8,0,0,0,16,0V168h29.82a16,16,0,0,0,15-10.53l26.19-72A16,16,0,0,0,229.11,70.82ZM110.68,152,97.58,80h60.84l-13.1,72ZM40,80H81.32l13.09,72H66.18Zm149.82,72H161.59l13.09-72H216Z";

/** Just the antenna loop at the top of the tower. */
export const ANTENNA = "M136,64V32h16a8,8,0,0,0,0-16H104a8,8,0,0,0,0,16h16V64Z";
