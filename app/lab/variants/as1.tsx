...
// it's exactly Phosphor — but the staff and serpent move as one undifferentiated
// shape, with no sense of the symbol being raised or coming alive.
const snap: Variants = {
  normal: { scale: 1, opacity: 1, transition: RETURN_TRANSITION },
  animate: { scale: [0.7, 1], opacity: [0.4, 1], transition: springPop },
};

export const Asclepius1 = forwardRef<IconHandle, IconProps>(function Asclepius1(
  { size = 28, style, ...props },
  ref,
) {
  ...
  return (
    <div {...props} {...bind} style={{ display: "inline-flex", overflow: "hidden", ...style }}>
      <motion.svg
        ...
        animate={controls}
      >
        <motion.path variants={reduced ? undefined : snap} style={origin} d={ASCLEPIUS} />
      </motion.svg>
    </div>
  );
});
