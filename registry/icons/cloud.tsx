...
//
// The drift is ambient (it repeats unattended), so `repeat` is gated on `ambient` from
// useHover() and threaded in as motion's `custom`. A reduced-motion visitor still gets one
// full drift on hover — the preview they asked for — and then it rests.
const cloud: Variants = {
  normal: { x: 0, y: 0, transition: RETURN_TRANSITION },
  animate: (ambient: boolean) => ({
    x: [0, 8, -8, 0],
    y: [0, -3, 0, 0],
    transition: { duration: 4.2, ease: "easeInOut", repeat: ambient ? Infinity : 0, repeatType: "loop" },
  }),
};
